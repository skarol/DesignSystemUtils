...
public struct ControlState: OptionSet, Hashable {
    /// Normal UI control's state. Applicable when no user action is being performed and control is enabled.
    public static let normal = ControlState(rawValue: 1)
    /// Highlighted UI control's state. Applicable when user taps/presses and control is enabled.
    public static let highlighted = ControlState(rawValue: 2)
    /// Disabled UI control's state. Applicable when control isn't enabled.
